...
| `/quiet`           | Keep routine wakes off the captain's conversation while present: on Pi and on a non-Pi home with an [attended supervision host](docs/supervision-host.md), this is already the ordinary posture; elsewhere `/quiet` starts a daemon that stays active through ordinary chat until `/quiet off` |
