...
The Pi supervision branch ([`pi-supervision-branch.md`](../pi-supervision-branch.md)) is Pi's in-process conversation and does not run on omp; an omp home with `config/supervision-host` follows the [supervision host protocol](supervision-host.md), including its attended pass-through and away session.
